...
public class VisionSubsystem extends SubsystemBase {
	...
	public void periodic() {
		...
		if (mt1 != null) {
			...
			if(RobotState.isEnabled() && (Math.abs(pose.getY() - YFilter.calculate(pose.getY())) > 0.5 || Math.abs(pose.getX() - xFilter.calculate(pose.getX())) > 0.5)){
				doRejectUpdate = true;
			}
		...
		Logger.recordOutput("Odom/limelight/limelight_theta_median", rFilter.calculate(this.robotToField.getRotation().getRadians()));
